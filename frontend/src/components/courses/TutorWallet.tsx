    ...
    <div className="min-h-screen md:px-8 md:pt-8 pt-10">
        ...
        >
            <div className="flex items-center md:justify-center gap-4 mb-4 md:mb-0">
                <div className="text-yellow-700 text-4xl">₹</div>
                <div className="font-sans">
                    ...
                    <p className="text-2xl md:text-3xl font-semibold text-green-500 font-poppins">
                        ₹{walletInfo?.deductedAmount || '0.00'}
                    </p>
                </div>
            </div>
        ...
        <div className="w-full mt-8">
            ...
            {(!walletInfo || walletInfo.totalAmount?.length === 0) ? (
                <div className="flex justify-center items-center h-64">
                    <p className="text-lg text-gray-500">No wallet transaction has been made.</p>
                </div>
            ) : (
                <div className="mb-4">
                    <input
...
{walletInfo?.walletDetails && walletInfo.walletDetails.length > 0 && (
                ...
                >
                    <table className="md:min-w-full bg-custom-gradient border border-gray-200 rounded-2xl shadow-md">
                        <thead className="bg-opacity-60 bg-gray-800 text-white uppercase text-sm font-semibold border-b border-gray-300">
                            <tr>
                                <th className="py-3 px-4 text-left cursor-pointer" onClick={() => handleSort('studentName')}>
                                    Name {sortColumn === 'studentName' && (sortDirection === 'asc' ? '▲' : '▼')}
                                </th>
                                <th className="py-3 px-4 text-left cursor-pointer" onClick={() => handleSort('courseTitle')}>
                                    Course Title {sortColumn === 'courseTitle' && (sortDirection === 'asc' ? '▲' : '▼')}
                                </th>
                                <th className="py-3 px-4 text-left cursor-pointer" onClick={() => handleSort('amountPaid')}>
                                    Amount Paid {sortColumn === 'amountPaid' && (sortDirection === 'asc' ? '▲' : '▼')}
                                </th>
                                <th className="py-3 px-4 text-left cursor-pointer" onClick={() => handleSort('paymentDate')}>
            ...
            {walletInfo?.walletDetails && walletInfo.walletDetails.length > 0 && (
                <div className="flex items-center justify-center mt-6 space-x-4">
                    ...
                    <div className="flex items-center justify-center">
                        <span className="text-lg font-semibold text-gray-700">{page}</span>
                        <span className="mx-2 text-gray-500">/</span>
                        <span className="text-lg font-semibold text-gray-700">{totalPages}</span>
                    </div>
